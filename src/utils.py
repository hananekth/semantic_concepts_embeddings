...
def load_vectors(mv_path, words=None):
    ...
    if words is not None:
        for w in words:
            ws = w + ".pt"
            if os.path.join(mv_path+"bert-base-cased",ws):
                vec = torch.load(os.path.join(mv_path+"bert-base-cased",ws)).tolist()
                logging.info("Vecteur lu " + str(ind))
    ...
    if filename.endswith(".txt"):
        with open(filename, 'r', encoding="utf-8") as f:
            for line in f:
                ...
                if len(items) < 100:
                    continue
                embeddings[items[0].replace(' ', '_')] = [float(i) for i in items[1:]]
